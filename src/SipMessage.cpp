...
void SipMessage::parse() {
    ...
    if (!isValidMessage()) {
        throw std::runtime_error("Неверное сообщение.");
    }
}
...
void SipMessage::setFrom(const std::string &value) {
...
void SipMessage::setTo(const std::string &value) {
...
std::string SipMessage::extractNumber(const std::string &header) {
